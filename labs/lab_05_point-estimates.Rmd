...
# Repeated Experiments: Simulation  
...
Let's assume that the mean and SD for the original sample is the "true parameters" i.e., $\mu$ = `r bt_mean` and $\sigma$ = `r bt_sd`. This is most certainly not the case, but it's a good enough estimate for now.  
...
Instead, we can use the power of R to automate this for us with minimal typing. Don't worry too much about what the code below is doing. Essentially, I'm going to use a "for loop" to repeat our sampling procedure 100 times. You will need to do something similar in the homework assignment, but you can just copy the below code and make a couple of minor modifications. If you have any questions on the homework assignment, ask we and I can help you with it in class. 

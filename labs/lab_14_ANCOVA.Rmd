...
add hypotheses, and about section
Make homework and solution 
Do I need more intro??  
...
For this lab, we will be using the `dplyr`, `ggplot2`, `Sleuth3` and the `car` packages. Make sure all packages are installed on your computer (you may need to install `Sleuth3` and `car`), and then run the following code:
...
* `StressWell-watered`: This is the estimated difference in intercepts for the two treatment groups. Estimate = $\beta_0 + \beta_{well-watered} = 8.49 + 1.916 = 10.41$; This is the average yield for the `Well-watered` treatment when Ozone = 0.  
* `O3:Stresswell-watered`: This is the estimated difference in slopes for the two treatment groups (but note that the p-value > 0.05, so we fail to reject the NULL that this coefficient is 0). Estimate = $-6.47 - 1.35 = -7.82$.  

$$ \text{Yield}_{well-watered} = (8.49 + 1.916) - (6.47 - 1.35) * \text{Ozone} = 10.41 - 7.82 * \text{Ozone}$$

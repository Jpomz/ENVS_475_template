...
* Download the `shrub-volume-data.csv` and the `marine.csv` file from D2L and save them in your `data/` folder
...
marine <- read.csv("data/marine_hw03.csv")
```
...
Get familiar with the data by using `dplyr` to complete the following tasks.
...
If the file `surveys.csv` is not already in your working `data/` folder then download a copy.

Load the file into R using `read.csv()`.
* Use the example from the `shrub_data` as a guide

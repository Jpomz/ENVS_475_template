---
title: "Homework 01: Introduction to R"
...
no_eggs <- 6
egg_mass_g <- 14
egg_mass_total <- no_eggs * egg_mass_g
egg_mass_total
...
***
You try it - modify the code above removing the `" "` around the different occassions within the `( )`. What does the Error message tell you?
***
...
Print out the `mixed` object in the console. What does it look like? What happend to the `numbers` and the `characters` in the vector?

You can also use the `c()` function to add other elements to your vector:

```{r echo = TRUE}
# first, recal what value you stored in "x"
x
...
Every function takes a different set of arguments and in many cases you will need to look up what those arguments are. The best way to get help for a specific function is to type a question mark followed by the function name, which will bring up a help page in the `Help` tab in R studio. For example, the `round` function rounds a number to a specified number of decimal places. This is a useful function when we don't want to print a really large number of digits:
...
Occasionally, some packages are stored in other places (e.g., github). These packages can be installed using different functions. For example, this course is largely based on a course created by Clark Rushing at the University of Georgia. He created a package for the course that contains small data sets which we will use in labs throughout the semester. His package is stored on github and can be installed by running:
...
We can see that `jayData` contains eight columns: `x`, `y`, `elevation`, `forest`, `chaparral`, `habitat`, `seeds`, and `jays`. We'll learn more about what each of these columns represents later in the semester. 
Several other useful functions for investigating the structure of data frames are `names()` `str()` and `summary()`
...
**Quick note** I used the `head()` function around the `select()` function above to shorten the display of the `jayData` to save room. Nesting functions like this (`head(select(...))`) can be very useful. However, it can also be confusing because you have to read the functions "inside out". Likewise it can be easy to miss or add additional `()` to either the beginning or the end of the function call. To help reduce this risk, I recommend turning on "rainbow brackets":
...
Those are very different results and knowing what the output will be can be useful when deciding the best way to accomplish your task.  
...
Notice the need for two equals signs (`==`) when telling `R` we want the row where `habitat` equals `Oak`. A related task might be to filter out all the rows *that are not* `Oak`. In this case we would use the `!=` operator which can be read "is not equal to":
...
It is also possible to use the "or" operator (`|`) when we want to select observations which meet different values. For example, we want observations of less than 800 *OR* > 1200:

```{r}
head(filter(jayData, elevation < 800 | elevation > 1200))
```

We can also filter out observations based on multiple columns using the "and" (`&`) operator. For example, we want the rows which have an elevation less than 1000, **AND** are in the Oak habitat:

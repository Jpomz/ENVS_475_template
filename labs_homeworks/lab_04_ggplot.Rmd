...
library(ggplot2)
acacia <- read.csv(here::here("data/uhuru_acacia.txt"), sep="\t", na.strings = c("dead"))
```
...
* There are 3 different treatments:
  * The top-left image shows Megaherbivore exclosures, which use wires 2m high to keep out elephants
...
* But first, take a look at the `uhuru_acacia.txt` data sets by opening them. 
* * *Click on Acacia Dataset to Open in Text Editor*
* This data is tab separated, so we'll need to tell that to R so it will be read in correctly. 
  * The other, more common form of storing data is in a `.csv`, or comma separated table. 
  * Recall that we used `read.csv()` in our `dplyr` homework. This is the main one we will throughout this course. 
  * Learning how to read in other file types is a useful skill. 
...
```{r, eval=FALSE}
acacia <- read.csv("data/uhuru_acacia.txt", sep="\t")
```

Whenever you load a dataset into R, it is always a good idea to take a look at it to familiarize yourself with the structure. Run the following lines of code in RStudio (I have removed the output to save space here): 
```{r, eval=FALSE}
names(acacia)
head(acacia)
tail(acacia)
...
```{r, eval=FALSE}
acacia <- read.csv("data/uhuru_acacia.txt", sep="\t", na.strings = c("dead"))
```

* If we open the resulting table (i.e., `View(acacia)`) we can see that it includes information on:
...
* If you hvaen't already loaded the UHURU data, make sure to do it now: 
```{r, eval=FALSE}
acacia <- read.csv("data/uhuru_acacia.txt", sep="\t", na.strings = c("dead"))
```
...
* For example, in the `labs()` call above, I hit enter after the `x = "...",`
  * and the `y = "...",
  * etc. 
...
> Do Task 2.3 in in homework 04: `ggplot`

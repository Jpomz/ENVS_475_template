...
Before we begin, make sure to download the data from D2L. The data sets are called `uhuru_acacia_lab2.txt` and `uhuru_tree_lab2.txt`. Make sure to place these 2 files in the `data/` folder in your R Project, otherwise you will need to modify the code below in order to have the code run successfully. 
...
* So far we've been working with datasets that are built into R, or making our own small data sets within our scripts. 
* Now we will need to read in the datasets into R. 
* But first, take a look at the `uhuru_acacia_lab2.txt` and `uhuru_tree_lab2.txt` datasets by opening them. 
...
* * The other, more common form of storing data is in a `.csv`, or comma separated table. We will go over how to read those file types into R later in the course. 
...
```{r}
acacia <- read.csv("data/uhuru_acacia_lab2.txt", sep="\t")
```

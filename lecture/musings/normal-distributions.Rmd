...
![The Empirical Rule. (image credit Open Intro Statistics)](68-95-997.png){width=70%}
...
![rough sketc](sketch.png)
The curve is centered at our mean of 10, and I added a few marks indicating 1 and 2 standard deviations away from the mean: $10 \pm 2$ and $10 \pm 2*2$. 
...
![Sketch with observations](sketch-obs.png)
Doing this can be a quick estimate of where the observations fall, and can help us to check our answers. 
...
### Probability of an *exact* number 
...
### Probability of more-or-less than a value
...
ggplot(data.frame(x = x1), aes(x)) +
  ...
  labs(title = "Area shaded to the left of our value (in blue)")
```
...
ggplot(data.frame(x = x1), aes(x)) +
  ...
  theme_bw() +
  theme(legend.position = "none")+
  labs(title = "Area shaded to the right of our value (in blue)")
```
...
ggplot(data.frame(x = x1), aes(x)) +
  ...
  theme_bw() +
  theme(legend.position = "none")+
  labs(title = "Area shaded between values (in blue)")
```
...
### What value is a quantile of our data?
...
- SAT scores are ${Normal}( \mu = 1500, \sigma = 300)$. ACT scores are ${Normal}(\mu = 21, \sigma = 5)$.
...
![SAT and ACT SCores. (image credit Open Intro Statistics)](sat-act.png){width=50%}
